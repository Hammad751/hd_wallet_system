...
export class FlushService {
    ...
    private async processSingleAddressFlush(userAddress: string, request: FlushRequest): Promise<string | null> {
        try {
            ...
            const balanceBigInt = ethers.parseUnits(parseFloat(balanceStr).toFixed(18), 18);

            // 3. Dynamic Fee Estimation
            const gasFee = await this.getDynamicGasFee(adapter, chain as ChainType, request.token);
            const gasFeeBigInt = ethers.parseUnits(gasFee.toFixed(18), 18);
            const minThreshold = gasFeeBigInt * BigInt(2);

            if (balanceStr === '0' || !balanceStr) return null;

            if (balanceBigInt <= minThreshold) return null;

            // 4. Calculate final amount (subtract fee only if flushing native token)
            let amountToFlush = balanceBigInt;
            if (!request.token) {
                const gasWithBuffer = (gasFeeBigInt * BigInt(120)) / BigInt(100);
                amountToFlush = balanceBigInt - gasWithBuffer;
            }

            if(amountToFlush <= BigInt(0)) return null;

            // 5. Execute Transfer
            const txHash = await this.transferService.transfer({
                fromAddress: userAddress,
                toAddress: request.hotWalletAddress,
                amount: ethers.formatUnits(amountToFlush.toString(), 18),
